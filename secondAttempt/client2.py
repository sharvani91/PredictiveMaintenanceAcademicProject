...
def run_client(device_name, X_scaled_train_balanced, y_train_balanced, df_train_balanced, device_map):
    ...
    def build_model():
        model = Sequential([
            Dense(256, activation='relu', input_shape=(X_dev.shape[1],), kernel_regularizer=l2(0.001)),  # Reduced L2
            BatchNormalization(),
            Dropout(0.3),  # Reduced from 0.4 to 0.3
    ...
    class FedProxClient(fl.client.NumPyClient):
        ...
        def fit(self, parameters, config):
            ...
            lr = initial_lr * (0.95 ** (self.round - 1))  # Slower decay
            ...
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr, clipnorm=1.0)  # Increased from 0.5
            model.compile(optimizer=optimizer, loss=proximal_loss, metrics=['accuracy'])

            # Split data for validation
            from sklearn.model_selection import train_test_split
            X_train_dev, X_val_dev, y_train_dev, y_val_dev = train_test_split(X_dev, y_dev, test_size=0.1, random_state=42)
            history = model.fit(
            ...
            logger.info(f"Client {device_name}: Validation Loss: {val_loss:.4f} for round {self.round}")
            if val_loss < self.best_val_loss:
                self.best_val_loss = val_loss
            else:
                logger.warning(f"Client {device_name}: Validation loss increased. Consider stopping early.")
            ...
            for i in range(len(weights)):
                weights[i] = np.clip(weights[i], -1.0, 1.0)  # Increased from [-0.5, 0.5]
            model.set_weights(weights)

            logger.info(f"Client {device_name}: returning weights with {len(weights)} arrays")
            return weights, len(X_train_dev), {"loss": float(history.history['loss'][-1])}
