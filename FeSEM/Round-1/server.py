...
# Global variables
global_weights = None
personalized_models = {}  # Store personalized models for each client
metrics_history = {"loss": [], "accuracy": [], "f1_score": []}
momentum = 0.9  # Momentum for server-side updates
velocity = None
...
class FeSEM(fl.server.strategy.Strategy):
    def __init__(self, initial_parameters, min_fit_clients=3, min_evaluate_clients=3, min_available_clients=3):
        ...
        self.client_accuracies = {}  # Store client accuracies for adaptive weighting
    ...
    def initialize_parameters(self, client_manager):
        ...
        # Initialize personalized models for clients (will be populated as clients connect)
        personalized_models = {}
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        ...
        # Wait for clients in every round
        timeout = 300
    ...
    def aggregate_fit(self, server_round, results, failures):
        """Aggregate client updates with adaptive weighting and update personalized models."""
        global global_weights, velocity, personalized_models
        if not results:
            logger.warning(f"Round {server_round}: No results received. Failures: {len(failures)}")
            return None, {}

        # Compute adaptive weights based on client accuracies
        total_accuracy = sum(self.client_accuracies.get(str(client.cid), 0.0) for client, _ in results)
        if total_accuracy == 0:
            total_accuracy = 1.0  # Avoid division by zero
        client_weights = {str(client.cid): (self.client_accuracies.get(str(client.cid), 0.0) / total_accuracy) for client, _ in results}
        ...
        # Update personalized models with a mix of local and global updates
        for client, res in results:
            cid = str(client.cid)
            local_weights = fl.common.parameters_to_ndarrays(res.parameters)
            personalized_models[cid] = [
                0.7 * lw + 0.3 * gw for lw, gw in zip(local_weights, global_weights)
            ]
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        """Aggregate evaluation metrics from clients and store accuracies."""
        ...
        for client, res in results:
            ...
            # Store client accuracy for adaptive weighting in the next round
            self.client_accuracies[cid] = res.metrics["accuracy"]
    ...
    def evaluate(self, server_round, parameters):
        """Evaluate the global model on the server side using the test set."""
...
def run_server():
    ...
    try:
        fl.server.start_server(
            ...
            config=fl.server.ServerConfig(num_rounds=15, round_timeout=300),
            strategy=strategy
        )
