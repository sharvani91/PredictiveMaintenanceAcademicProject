...
class IFCAStrategy(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        for cluster_id in range(self.num_clusters):
            ...
            for layer in zip(*updates):
                ...
                weights = np.array([n / total_samples for n in num_examples])
                weights = weights.reshape(-1, *[1] * (len(layer_updates.shape) - 1))
    ...
    def configure_evaluate(
        ...
        """Configure clients for evaluation using cluster-specific models."""
        ...
        for attempt in range(max_retries):
            ...
            for client in sample_clients:
                ...
                cluster_weights = cluster_models[cluster_id]
                # Temporarily avoid sending all_cluster_weights to simplify payload
                parameters = fl.common.ndarrays_to_parameters(cluster_weights)
                config = {
                    "round": server_round,
                    "cluster_id": cluster_id,
                    # "all_cluster_weights": all_cluster_weights  # Commented out for now
                }
                logger.info(f"Round {server_round}: Sending evaluate instructions to client {cid} for cluster {cluster_id}")
                eval_ins.append((client, fl.common.EvaluateIns(parameters, config)))

            # Attempt evaluation and check results in aggregate_evaluate
    ...
    def aggregate_evaluate(
        ...
        failures: List[Tuple[fl.server.client_proxy.ClientProxy, fl.common.typing.EvaluateRes]]
    ) -> Tuple[Optional[float], Dict[str, float]]:
        """Aggregate evaluation metrics and reassign clients to clusters."""
        ...
        for client, failure in failures:
            try:
                # Attempt to get more information about the failure
                cid = str(client.cid)
                logger.error(f"Round {server_round}: Evaluation failed for client {cid}. Failure details: {failure}")
            except Exception as e:
                logger.error(f"Round {server_round}: Could not retrieve failure details for client {cid}: {e}")
        ...
        # Temporarily skip cluster reassignment since all_cluster_weights is not sent
        # We can reintroduce this once evaluation is stable
        # for client, res in results:
        #     cid = str(client.cid)
        #     client_losses = {}
        #     for cluster_id in range(self.num_clusters):
        ...
        #     logger.info(f"Round {server_round}: Client {cid} assigned to cluster {best_cluster} with loss {client_losses[best_cluster]:.4f}")
...
def run_server():
    ...
    try:
        fl.server.start_server(
            ...
            config=fl.server.ServerConfig(num_rounds=30, round_timeout=900),
            strategy=strategy,
